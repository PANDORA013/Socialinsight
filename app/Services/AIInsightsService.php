...
class AIInsightsService
{
    ...
    protected function calculateViralityScore($data)
    {
        ...
        if ($avgEngagement > 10000) return 'Very High';
        if ($avgEngagement > 5000) return 'High';
        if ($avgEngagement > 1000) return 'Moderate';
        return 'Low';
    }
    ...
    protected function getTopYouTubeVideosByViews($data, $limit = 3)
    {
        ...
        $youtubeVideos = array_filter($data, fn($item) => 
            strtolower($item['platform'] ?? '') === 'youtube'
        );
    ...
    protected function getTopCommentsByLikes($data, $limit = 3)
    {
        ...
        $postsWithLikes = array_filter($data, fn($item) => 
            ($item['likes'] ?? 0) > 0
        );
